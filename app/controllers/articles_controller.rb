class ArticlesController < ApplicationController
  ...
  # The edit action fetches the article from the database, and stores it in @article so that it can be used 
  # when building the form. By default, the edit action will render app/views/articles/edit.html.erb.
  def edit
    @article = Article.find(params[:id])
  end

  # The update action (re-)fetches the article from the database, and attempts to update it with the submitted form data 
  # filtered by article_params. If no validations fail and the update is successful, the action redirects the browser to the article's 
  # page. Else, the action redisplays the form — with error messages — 
  # by rendering app/views/articles/edit.html.erb.
  def update
    @article = Article.find(params[:id])
